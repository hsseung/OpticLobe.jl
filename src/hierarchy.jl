...
Maps visual neuron classes to their constituent type families.

Hierarchical classification system for intrinsic visual neurons with 5 main classes:
- **"receptor"**: Photoreceptor families ("R1-6", "R7-8")  
- **"columnar"**: Column-specific neurons ("L", "Mi", "Tm", "T2", etc.)
- **"interneuron"**: Intra-neuropil non-columnar neurons ("Dm", "Pm", "Li", etc.)
- **"x-tangential"**: Cross-neuropil tangential neurons ("Lat", "MLt", etc.)
- **"x-amacrine"**: Cross-neuropil amacrine neurons ("LMa", "MLLPa")

# Examples
```julia
class2families["receptor"]        # ["R1-6", "R7-8"]
class2families["columnar"]        # ["L", "C", "Lawf", "T1", "Mi", "Tm", ...]
keys(class2families)              # All class names
length(class2families["columnar"]) # Number of columnar families
```

# Notes
- Top level of the class → family → type hierarchy
- Each class contains multiple type families based on neuropil organization
- Used to organize the 230 intrinsic visual neuron types
- Basis for `family2types` mapping
"""
class2families = OrderedDict{String, Vector{String}}()
...
Maps type families to their constituent individual cell types.

Second level of the class → family → type hierarchy, where each family contains 
specific cell types sharing common morphological and functional properties.

# Examples
```julia
family2types["Tm"]            # ["Tm1", "Tm2", "Tm3", ...]
family2types["R7-8"]          # ["R7", "R8", "R7-DRA", "R8-DRA"]
family2types["Dm"]            # ["Dm1", "Dm2", "Dm3", ...]
keys(family2types)            # All family names
length(family2types["Tm"])    # Number of Tm types
```

# Notes
- Generated by filtering `intrinsictypes` using family-specific prefixes
- Each family groups types with similar naming patterns and functions
- All 230 intrinsic types are included across families
- Used for hierarchical analysis and organization of visual neuron types
- Families correspond to entries in `class2families` values
"""
family2types = OrderedDict{String, Vector{String}}()
